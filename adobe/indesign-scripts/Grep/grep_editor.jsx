...
function grep_edit () {
	...
	GREPeditor = new Window ('palette', GrepEditorName, undefined, {resizeable: true});
		...
		GREPeditor.editWindow = GREPeditor.add ('edittext', undefined, decodeURI (decode_grep (PREVIOUS_RUN.w_grep)), {multiline: true, wantReturn: true, scrolling: true, name: 'grep_edit_window'});
		GREPeditor.editWindow.alignment = ['fill', 'fill'];
		GREPeditor.editWindow.preferredSize = PREVIOUS_RUN.editWindowSize || [500, 400];
	...
	GREPeditor.highlight.onClick = function () {
		...
		if (GREPeditor.highlight.pressed == '1') {
			...
			GREPeditor.editWindow.onChanging = function () {/* disable the handler */}
		} else {
	...
	function GREP_highlighter (name) {
		...
		app.activeDocument.conditions.add ({name: name, indicatorColor: [255,255,0], indicatorMethod: ConditionIndicatorMethod.USE_HIGHLIGHT});
		...
		gr_style.onClick = function () {
			...
			if (w1.show () == 1) {
				...
				for (var i = 0; i < pstyles.selection.length; i++){
					try {
						pstyles.selection[i].style.nestedGrepStyles.add ({grepExpression: gr, appliedCharacterStyle: cs})
					} catch (e) {
						alert (e.message)
					}
				}
	...
	close_window.onClick = function () {
		...
		var windowState = {
			w_location: [GREPeditor.frameLocation[0], GREPeditor.frameLocation[1]],
			editWindowSize: [GREPeditor.editWindow.size[0], GREPeditor.editWindow.size[1]],
			wildcard_window: wildcard_location(),
			w_grep: encodeURI (encode_grep_for_storage (GREPeditor.editWindow.text)),
		}
	...
	GREPeditor.onShow = function () {
		...
		GREPeditor.editWindow.minimumSize.height = GREPeditor.buttons.size[1];
	}
...
//================================================================================================
	...
	//------------------------------------------------------------------------
	// Return the findWhat and changeTo strings from the query xml
	// (We don't do anything with the changeTo string, but that might change)
	
	function getQueryData (f) {
		f.open ('r');
		//f.encoding = 'utf-8';  // Don't use this, some content isn't read properly in this mode
		var xml = new XML (f.read());
		f.close();
		try {
			return {
				name: f.name.replace(/\.xml$/,''),
				find: String (xml.Description.FindExpression.@value),
				change: xml.Description.ReplaceExpression.@value
			}
		} catch (e) {
			return {
				name: f.name.replace(/\.xml$/,''),
...
function wildcard_location () {
	...
	return [w.frameLocation[0], w.frameLocation[1]];
}
