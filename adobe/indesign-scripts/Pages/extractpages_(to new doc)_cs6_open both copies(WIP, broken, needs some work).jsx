	...
	if(dlg.show()==true)
	{
		...
		else
		{
			...
			for(w=0; w<W;w++)
			{
				if(sepbox.checkedState==true)
				{
					var exportdocname = prompt("Choose a name for the document","_Pg" +(pgStart.editValue+w) );
				}
				else
				{
					var exportdocname = prompt("Choose a name for the document","_Pg"+pgStart.editValue+"_to_Pg_"+pgEnd.editValue);
				}
				...
				var exportdoc = doc.saveACopy(File(saveextractfolder + "_" + exportdocname + ".indd"));
				app.open(File(exportdoc));
//EDITED HERE TO KEEP DOC OPEN				exportdoc.close(SaveOptions.no);
