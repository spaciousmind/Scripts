...
- In selected text change each paragraph style that's "P1" to "P2" (quicker than using indesigns find/change) :winking_face:
...
A nasty use case you should understand is this: Imagine "If -this- paragraph's style is  followed by [-ANY-Style-] THEN change this paragraph to  AND its next paragraph to ." ... This woul...
  ...
  var sts = []; // an array of stories
    ...
    if (s.length == 0) { // no selection ? then do all paras of layer or document 
        ...
        for (i=0; i < sts.length; i++) {
            ...
            for (j=0; j < sts.paragraphs.length; j++) {

                ps.push(sts.paragraphs);
...
function myDisplayDialog(ad, desc){
  ...
  for (j=i-1; j >= 0; j--) {styleArrayMarked = markOfNotFound + styleArrayMarked;}
...
function paragraphStyleChanger(ad, ps) {
  ...
  // fsp = find_second_paragraph; csp = change_second_paragraph; ffp = find_first_paragraph; cfp = change_first_paragraph;
    ...
    // here is the logic :slightly_smiling_face:
    ...
    for (i = 0; i < dolist_fp.length; i++) {
        ...
        dolist_fp.applyParagraphStyle(cfp, false);

    }

    for (i = 0; i < dolist_sp.length; i++) {

        dolist_sp.applyParagraphStyle(csp, false);
...
function getNestedListOfShortendedStyles (aps) {
  ...
  for (i = 0; i < aps.length; i++) {
        ...
        styleArray.push(getNestingPath(aps, aps.name.shorten(20)));  // recurse down style nesting and collect parents-names
            ...
            switch (s.constructor.name) {
                ...
                if (s.textPaths.length > 0) {
                    ...
                    for (var j = 0; j < s.textPaths.length; j++) {
                        ...
                        if (s.textPaths.parentStory.itemLink == null) {

                            if (!IsObjInArray(s.textPaths.parentStory, myStories)) {

                                myStories.push(s.textPaths.parentStory);
