...
HRfiletoJobFolder();  // export scr proof to job folder
...
alert(Math.round((time2-time1)/1000) +" seconds" +"\n\n" + "exported: " + docName + "__HR.pdf & HR_proof sent to printer");
...
function HRfiletoJobFolder(){
  var myDocument = app.documents.item(0);
	var myPDFExportPreset = app.pdfExportPresets.item("Inferno Print File (no trim marks)");
  myDocument.exportFile(ExportFormat.pdfType, File(jobFolder + "/" + docName + "_HR.pdf"), false, myPDFExportPreset);}
